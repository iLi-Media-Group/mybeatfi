/*
  # Sync Proposals Enhancement

  1. New Columns
    - `client_status` (text) - Tracks client's response to proposals
    - `payment_status` (text) - Tracks payment status for accepted proposals
    - `invoice_id` (text) - Stores reference to payment invoice
    - `payment_date` (timestamp) - Records when payment was made

  2. New Function
    - `handle_sync_proposal_payment()` - Processes payments for sync proposals
    - Updates proposal status and distributes earnings to producers

  3. Security
    - Updates RLS policies for sync_proposals table
*/
