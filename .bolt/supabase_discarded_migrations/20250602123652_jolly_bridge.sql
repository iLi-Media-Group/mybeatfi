...
BEGIN
  ...
  -- Calculate no sales bucket (2% of membership revenue)
  no_sales_bucket := membership_revenue * (settings.no_sales_bucket_rate / 100.0);
  ...
  LOOP
    ...
    IF producer_record.monthly_sales > 0 THEN
      ...
      producer_share := (producer_record.monthly_sales / (SELECT SUM(amount) FROM producer_transactions WHERE type = 'sale' AND created_at >= date_trunc('month', month_date) AND created_at < date_trunc('month', month_date) + interval '1 month')) * (membership_revenue - no_sales_bucket);
    ...
    ELSE
      ...
      producer_share := no_sales_bucket / inactive_producers;
