/*
# Compensation Settings and Producer Earnings

1. New Tables
   - `compensation_settings`: Stores rates and parameters for producer compensation
   - `producer_balances`: Tracks producer earnings and balances
   - `producer_transactions`: Records all financial transactions for producers

2. Functions
   - `calculate_producer_earnings()`: Calculates earnings when a sale is made
   - `distribute_membership_revenue()`: Distributes membership revenue to producers

3. Changes
   - Updates standard rate to 75% (from 70%)
   - Updates sync fee rate to 90% (from 85%)
*/
...
-- Enable RLS on producer_balances
ALTER TABLE producer_balances ENABLE ROW LEVEL SECURITY;

-- Create RLS policies for producer_balances
...
-- Enable RLS on producer_transactions
ALTER TABLE producer_transactions ENABLE ROW LEVEL SECURITY;

-- Create RLS policies for producer_transactions
...
BEGIN
  IF EXISTS (SELECT 1 FROM pg_tables WHERE schemaname = 'public' AND tablename = 'tracks') THEN
    -- Create the calculate_producer_earnings function
    ...
    -- Create a trigger for the calculate_producer_earnings function
    DO $$
    BEGIN
      IF NOT EXISTS (
    ...
    -- Create the distribute_membership_revenue function
