...
-- Add expiry_date to sales table if it doesn't exist
DO $$
BEGIN
...
-- Add license_duration to licenses table if it doesn't exist
DO $$
BEGIN
...
-- Create trigger for sales expiry date if sales table exists
DO $$
BEGIN
  IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'sales') THEN
