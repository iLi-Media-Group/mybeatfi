...
import { X, Music, Upload, Globe, Lock, Loader2, Trash2 } from 'lucide-react';
import { supabase } from '../lib/supabase';

interface EditPlaylistDialogProps {
  isOpen: boolean;
  onClose: () => void;
  onUpdatePlaylist: (name: string, description: string, isPublic: boolean, coverImageUrl: string | null) => void;
  playlist: {
    id: string;
    name: string;
...
export function EditPlaylistDialog({ isOpen, onClose, onUpdatePlaylist, playlist }: EditPlaylistDialogProps) {
  const [name, setName] = useState(playlist.name);
  const [description, setDescription] = useState(playlist.description || '');
  const [isPublic, setIsPublic] = useState(playlist.is_public);
  const [coverImageFile, setCoverImageFile] = useState<File | null>(null);
  const [coverImagePreview, setCoverImagePreview] = useState<string | null>(playlist.cover_image_url);
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setCoverImagePreview(playlist.cover_image_url);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      onUpdatePlaylist(name.trim(), description.trim(), isPublic, coverImageUrl);
    } catch (err) {
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            ...
            <div className="flex items-center space-x-4">
              <div className="w-24 h-24 rounded-lg overflow-hidden bg-white/10 relative group">
                {coverImagePreview ? (
                  <>
                    <img
